...
class indexer():
    ...
    def root(self):
        try:
            regex.clear()
            url = base64.b64decode ('aHR0cHM6Ly9yYXcuZ2l0aHVidXNlcmNvbnRlbnQuY29tL3dhdGNoZG9nczM2L3N0cmFpdGphY2tldC9tYXN0ZXIvdGV4dC93ZG0zdS93ZG0zdS54bWw=')
    ...
    def root_music(self):
        try:
            regex.clear()
            url = base64.b64decode ('aHR0cHM6Ly9yYXcuZ2l0aHVidXNlcmNvbnRlbnQuY29tL3dhdGNoZG9nczM2L3N0cmFpdGphY2tldC9tYXN0ZXIvdGV4dC9tdXNpYy9tdXNpYy54bWw=')
            self.list = self.watchdogs_list(url)
            for i in self.list: i.update({'content': 'addons'})
            self.addDirectory(self.list)
            return self.list
    ...
    def root_oneclick(self):
        try:
            regex.clear()
            url = base64.b64decode ('aHR0cHM6Ly9yYXcuZ2l0aHVidXNlcmNvbnRlbnQuY29tL3dhdGNoZG9nczM2L3N0cmFpdGphY2tldC9tYXN0ZXIvdGV4dC9vbmVjbGljay9vbmVjbGljay54bWw=')
            self.list = self.watchdogs_list(url)
            for i in self.list: i.update({'content': 'addons'})
            self.addDirectory(self.list)
            return self.list
    ...
    def root_swift(self):
        try:
            regex.clear()
            url = base64.b64decode ('aHR0cHM6Ly9yYXcuZ2l0aHVidXNlcmNvbnRlbnQuY29tL3dhdGNoZG9nczM2L3N0cmFpdGphY2tldC9tYXN0ZXIvdGV4dC9zd2lmdC9zd2lmdC54bWw=')
            self.list = self.watchdogs_list(url)
            for i in self.list: i.update({'content': 'addons'})
            self.addDirectory(self.list)
            return self.list
    ...
    def root_tvtap(self):
        try:
            regex.clear()
            url = base64.b64decode ('aHR0cHM6Ly9yYXcuZ2l0aHVidXNlcmNvbnRlbnQuY29tL3dhdGNoZG9nczM2L3N0cmFpdGphY2tldC9tYXN0ZXIvdGV4dC90dnRhcC90dnRhcC54bWw=')
            self.list = self.watchdogs_list(url)
            for i in self.list: i.update({'content': 'addons'})
            self.addDirectory(self.list)
            return self.list
    ...
    def root_porn(self):
        try:
            regex.clear()
            url = base64.b64decode ('aHR0cHM6Ly9yYXcuZ2l0aHVidXNlcmNvbnRlbnQuY29tL3dhdGNoZG9nczM2L3N0cmFpdGphY2tldC9tYXN0ZXIvdGV4dC9wb3JuL3Bvcm4ueG1s')
            self.list = self.watchdogs_list(url)
            for i in self.list: i.update({'content': 'addons'})
            self.addDirectory(self.list)
            return self.list
    ...
    def tvtuner(self, url):
        try:
            ...
            items = [ (str(i.get('season')),
             str(i.get('number')),
             i.get('name').strip(),
             i.get('airdate')) for i in items ]
            if preset == 'tvtuner':
    ...
    def search(self, url):
        try:
            mark = False
            if url == None or url == '':
                self.list = [{'name': 30702,
                  'action': 'addSearch'}]
                self.list += [{'name': 30703,
                  'action': 'delSearch'}]
            else:
                if '|SECTION|' in url:
        ...
        for item in items:
            try:
                ...
                if item.startswith('<item>'):
                    ...
                    action = 'directory'
                if action == 'play1' and reglist:
                    action = 'xdirectory'
                if not regdata == '':
                    self.hash.append({'regex': reghash,
                     'response': regdata})
                    url += '|regex=%s' % reghash
                ...
                if 'tvshow' in content and not url.strip().endswith('.xml'):
                    url = '<preset>tvindexer</preset><url>%s</url><thumbnail>%s</thumbnail><fanart>%s</fanart>%s' % (url,
                     image2,
                     fanart2,
                     meta)
                    action = 'tvtuner'
                if 'tvtuner' in content and not url.strip().endswith('.xml'):
                    url = '<preset>tvtuner</preset><url>%s</url><thumbnail>%s</thumbnail><fanart>%s</fanart>%s' % (url,
                     image2,
                     fanart2,
                     meta)
                    action = 'tvtuner'
    ...
    def movie_info(self, i):
        try:
            ...
            try:
                premiered = '%s-%s-%s' % (premiered[0][2], {'Jan': '01',
                  'Feb': '02',
                  'Mar': '03',
                  'Apr': '04',
                  'May': '05',
        ...
        if items == None or len(items) == 0:
            ...
            for i in items:
                try:
                    ...
                    if folder == False and '|regex=' not in str(i.get('url')) or folder == True and content in ('tvshows', 'seasons'):
                    ...
                    if content == 'movies':
                        ...
                        try:
                            cm.append((control.lang(30722).encode('utf-8'), 'RunPlugin(%s?action=addDownload&name=%s&url=%s&image=%s)' % (sysaddon,
