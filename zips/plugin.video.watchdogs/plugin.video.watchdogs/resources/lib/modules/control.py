...
def getWatchdogsVersion():
    return xbmcaddon.Addon('plugin.video.watchdogs').getAddonInfo('version')
...
def artPath():
    theme = appearance()
    return os.path.join(xbmcaddon.Addon('plugin.video.watchdogs').getAddonInfo('path'), 'resources', 'artwork', theme)
...
def artwork():
    if condVisibility('System.HasAddon(script.watchdogs.artwork)'):
...
def metaFile():
    # if condVisibility('System.HasAddon(script.watchdogs.metadata)'):
        # return os.path.join(xbmcaddon.Addon('script.watchdogs.metadata').getAddonInfo('path'), 'resources', 'data', 'meta.db')
...
def clean_settings():
    ...
    for addon_id in ('plugin.video.watchdogs', 'script.module.openscrapers'):
        try:
            removed_settings = []
            active_settings = []
